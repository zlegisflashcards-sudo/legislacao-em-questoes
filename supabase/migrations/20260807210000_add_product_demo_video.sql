...
drop function if exists public.admin_criar_produto(uuid,text,text,text,text,text,text,integer,boolean,text);
...
create function public.admin_criar_produto(
  ...
  p_video_demo_url text, p_ordem integer, p_ativo boolean,
...
begin
  ...
  insert into public.produtos (nome,tipo,slug,descricao,tipo_produto,hotmart_url,hotmart_product_id,video_demo_url,ordem,ativo,observacao_administrativa)
  values (pg_catalog.btrim(p_nome),p_tipo_produto,p_slug,p_descricao,p_tipo_produto,p_hotmart_url,p_hotmart_product_id,nullif(p_video_demo_url,''),p_ordem,p_ativo,p_observacao_administrativa)
...
begin
  perform public.admin_comercial_validar_contexto(p_ator_user_id);
  if p_dados is null or p_dados='{}'::jsonb or p_dados-array['nome','slug','descricao','tipo_produto','hotmart_url','hotmart_product_id','video_demo_url','ordem','ativo','observacao_administrativa']<>'{}'::jsonb then
...
revoke all on function public.admin_criar_produto(uuid,text,text,text,text,text,text,text,integer,boolean,text) from public, anon, authenticated;
grant execute on function public.admin_criar_produto(uuid,text,text,text,text,text,text,text,integer,boolean,text) to service_role;
