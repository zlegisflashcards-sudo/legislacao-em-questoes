...
-- Records é configurado explicitamente no produto. A composição continua vindo
-- exclusivamente de produto_leis; sem composição, a consulta retorna vazia.
create function public.obter_ranking_produto_records(
  p_produto_slug text,
...
revoke all on function public.obter_ranking_produto_records(text, uuid, integer) from public, anon, authenticated;
grant execute on function public.obter_ranking_produto_records(text, uuid, integer) to service_role;
