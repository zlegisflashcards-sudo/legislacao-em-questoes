...
alter table public.produtos
  add column if not exists records_enabled boolean not null default false,
  add column if not exists imagem_url text null;

update public.produtos
set records_enabled = true
where ativo = true
  and tipo_produto = 'edital'
  and slug in ('pmmasd', 'cbmmasd', 'pmerjsd');

drop function if exists public.admin_criar_produto(uuid,text,text,text,text,text,text,text,boolean,integer,boolean,text);
drop function if exists public.admin_criar_produto(uuid,text,text,text,text,text,text,text,boolean,integer,boolean,text,boolean,text);
...
create function public.admin_criar_produto(
  ...
  p_observacao_administrativa text, p_records_enabled boolean default false,
...
begin
  ...
  insert into public.produtos (nome,tipo,slug,descricao,tipo_produto,hotmart_url,hotmart_product_id,video_demo_url,destaque,ordem,ativo,observacao_administrativa,records_enabled,imagem_url)
  values (pg_catalog.btrim(p_nome),p_tipo_produto,p_slug,p_descricao,p_tipo_produto,p_hotmart_url,p_hotmart_product_id,nullif(p_video_demo_url,''),coalesce(p_destaque,false),p_ordem,p_ativo,p_observacao_administrativa,coalesce(p_records_enabled,false),nullif(p_imagem_url,''))
...
begin
  perform public.admin_comercial_validar_contexto(p_ator_user_id);
  if p_dados is null or p_dados='{}'::jsonb or p_dados-array['nome','slug','descricao','tipo_produto','hotmart_url','hotmart_product_id','video_demo_url','destaque','ordem','ativo','observacao_administrativa','records_enabled','imagem_url']<>'{}'::jsonb then
  ...
  update public.produtos set
    ...
    records_enabled=case when p_dados?'records_enabled' then (p_dados->>'records_enabled')::boolean else records_enabled end,
    imagem_url=case when p_dados?'imagem_url' then nullif(p_dados->>'imagem_url','') else imagem_url end
...
revoke all on function public.admin_criar_produto(uuid,text,text,text,text,text,text,text,boolean,integer,boolean,text,boolean,text) from public, anon, authenticated;
grant execute on function public.admin_criar_produto(uuid,text,text,text,text,text,text,text,boolean,integer,boolean,text,boolean,text) to service_role;
...
values ('records-images', 'records-images', true, 3145728, array['image/png','image/jpeg','image/webp'])
on conflict (id) do update set
  public = excluded.public,
  file_size_limit = excluded.file_size_limit,
  allowed_mime_types = excluded.allowed_mime_types;

drop policy if exists "Public can read records images" on storage.objects;
create policy "Public can read records images"
on storage.objects for select
using (bucket_id = 'records-images');
