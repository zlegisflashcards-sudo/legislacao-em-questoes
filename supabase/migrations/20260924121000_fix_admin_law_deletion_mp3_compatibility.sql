...
-- A migration de exclusão integral foi aplicada antes da coluna opcional
-- legiscast_audio_jobs.mp3_path. Não referencie essa coluna diretamente:
-- a leitura JSON é compatível tanto com o schema anterior quanto com o novo.
...
begin
  ...
  select coalesce(jsonb_agg(path order by path), '[]'::jsonb)
    ...
    from (
      ...
      union
      select (pg_catalog.to_jsonb(j) ->> 'mp3_path')
        from public.legiscast_audio_jobs j
       where j.lei_id = p_lei_id
         and (pg_catalog.to_jsonb(j) ? 'mp3_path')
         and coalesce(pg_catalog.to_jsonb(j) ->> 'mp3_path', '') <> ''
    ) paths;
