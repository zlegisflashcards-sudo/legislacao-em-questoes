...
import { mustChooseLawStudyContext, selectLawStudyContext, shouldShowLawStudyContextSelector } from "@/lib/law-study-context-selection";
...
export function LawStudyPageClient({ slug, ankiTutorialSettings, publicStudy }: { slug: string; ankiTutorialSettings: AnkiTutorialSettings | null; publicStudy?: LawStudyData }) {
  ...
  const [contextsLoaded, setContextsLoaded] = useState(Boolean(publicStudy));
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const [law, state, structure, availableContexts] = await Promise.all([
  ...
  if (!study || !campaign || (!publicStudy && (!sourceLaw || !contextsLoaded))) return <Frame publicMode={Boolean(publicStudy)}><p className="rounded-2xl border bg-white p-6 text-slate-600">Carregando sua lei…</p></Frame>;
  if (!publicStudy && contexts.length === 0) return <Frame publicMode={false}><p className="rounded-2xl border border-amber-200 bg-amber-50 p-6 text-amber-900">O conteúdo liberado para esta lei está indisponível no momento.</p></Frame>;
  const completed = campaign.status === "concluida";
  const activeCampaign = !publicStudy && campaign.status === "em_andamento";
  const hasCampaignSummary = completed || activeCampaign || Boolean(campaign.record);
  const statusLabel = completed ? "Concluída" : campaign.status === "em_andamento" ? "Em andamento" : "Não iniciada";
  const modeLabel = "Estudo Livre";
  ...
  const openAnki = searchParams.get("anki") === "1";
  ...
  const showContextSelector = shouldShowLawStudyContextSelector(contexts, requestedScopeId, requestedFullContext);
  ...
  const tree = sourceLaw ? buildTree(visibleStructure, visibleQuestions) : [];
  return <Frame publicMode={Boolean(publicStudy)}><div className="law-study-layout grid min-w-0 gap-5 sm:gap-6">
    <header className="min-w-0 rounded-3xl border border-blue-100 bg-white p-4 shadow-sm sm:p-7 law-study-hero">
    ...
    {!mustChooseContext ? <section className="law-study-surface min-w-0 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:p-7"><div className="flex min-w-0 flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div className="min-w-0"><p className="text-sm font-black uppercase tracking-wide text-blue-700">{modeLabel}</p><h2 className="mt-1 text-2xl font-black text-[#062a5f]">Estrutura da lei</h2><p className="mt-2 text-sm text-slate-600">Escolha livremente o conteúdo e estude no seu ritmo.</p></div><div className="grid w-full gap-3 sm:flex sm:w-auto sm:flex-wrap"><Link href={`/questoes/${encodeURIComponent(slug)}/estudar?teste=1${contextQuery}`} className="law-study-button-primary inline-flex min-h-11 items-center justify-center rounded-xl bg-blue-700 px-4 py-2 font-black text-white hover:bg-blue-600">🎯 Criar teste</Link>{tree.length > 0 ? <Link href={`/questoes/${encodeURIComponent(slug)}/estudar?livre=1${contextQuery}`} className="law-study-button-secondary inline-flex min-h-11 items-center justify-center rounded-xl border border-blue-700 px-4 py-2 font-black text-blue-700 hover:bg-blue-50">Estudar lei inteira</Link> : null}</div></div><div className="law-study-list mt-5 min-w-0 overflow-hidden rounded-2xl border border-slate-200"><div className="hidden items-center gap-3 border-b border-slate-200 bg-slate-50 px-4 py-3 text-xs font-black uppercase tracking-wide text-slate-500 sm:flex"><span className="flex-1">Nível</span><span>Questões</span></div>{tree.length ? tree.map((node) => <StructureTreeNode key={node.id} node={node} slug={slug} recorteId={selectedContext?.recorteId ?? null} />) : <RootDeck study={study} slug={slug} count={selectedContext?.questionCount ?? sourceLaw?.questions.length ?? 0} recorteId={selectedContext?.recorteId ?? null} />}</div></section> : null}
...
function StudyContextSelector({ slug, contexts, selected }: { slug: string; contexts: StudyContext[]; selected: StudyContext | null }) {
  ...
  return <section className="law-study-surface min-w-0 rounded-3xl border border-blue-100 bg-white p-4 shadow-sm sm:p-7"><p className="text-sm font-black uppercase tracking-wide text-blue-700">Como estudar</p><h2 className="mt-1 text-2xl font-black text-[#062a5f]">{contexts.length > 1 ? "Escolha como estudar" : selected?.recorteId ? `Estudo conforme ${selected.nome}` : "Lei completa"}</h2><p className="mt-2 text-sm text-slate-600">{contexts.length > 1 ? "Escolha o conteúdo que foi liberado para sua conta." : `${selected?.questionCount ?? 0} ${(selected?.questionCount ?? 0) === 1 ? "questão disponível" : "questões disponíveis"}.`}</p>{contexts.length > 1 ? <div className="mt-5 grid gap-3">{contexts.map((context) => <div key={context.recorteId ?? "completa"} className="law-study-choice flex min-w-0 flex-col gap-3 rounded-2xl border border-slate-200 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><p className="break-words font-black text-[#062a5f]">{context.nome}</p><p className="mt-1 text-sm text-slate-600">{context.questionCount} {context.questionCount === 1 ? "questão" : "questões"}</p></div><Link href={hrefFor(context)} className="law-study-button-primary inline-flex min-h-11 shrink-0 items-center justify-center rounded-xl bg-blue-700 px-4 py-2 font-black text-white hover:bg-blue-600">Estudar</Link></div>)}</div> : selected ? <Link href={hrefFor(selected)} className="law-study-button-secondary mt-5 inline-flex min-h-11 items-center justify-center rounded-xl border border-blue-700 px-4 py-2 font-black text-blue-700 hover:bg-blue-50">{selected.recorteId ? "Ver estrutura do recorte" : "Estudar lei completa"}</Link> : null}</section>;
}
...
function FreeStudyLabel({ name, count, framed = true }: { name: string; count: number; framed?: boolean }) { return <div className={`flex min-h-12 min-w-0 flex-1 items-start gap-2 px-3 py-2 sm:items-center ${framed ? "rounded-xl border border-blue-100 bg-white transition-colors group-hover:border-blue-300 group-hover:bg-blue-50" : ""}`}><span className="min-w-0 flex-1 break-words font-bold leading-5 text-[#075bc1] group-hover:text-[#062a5f]">{name}</span><span className="mt-1 shrink-0 self-start text-sm font-bold text-blue-700 sm:mt-0 sm:self-center">{count} {count === 1 ? "questão" : "questões"}</span></div>; }
function StructureTreeNode({ node, slug, recorteId, depth = 0 }: { node: TreeNode; slug: string; recorteId: string | null; depth?: number }) { const [open, setOpen] = useState(false); const href = `/questoes/${encodeURIComponent(slug)}/estudar?livre=1&structure_id=${node.id}${recorteId ? `&recorte_id=${encodeURIComponent(recorteId)}` : ""}`; return <div className="border-b border-slate-100 last:border-b-0"><div className="flex min-h-16 min-w-0 px-3 py-3 sm:px-4" style={{ paddingLeft: `${12 + depth * 24}px` }}><div className="group flex min-w-0 flex-1 items-start rounded-xl border border-blue-100 bg-white transition-colors hover:border-blue-300 hover:bg-blue-50 sm:items-center">{node.children.length ? <button type="button" onClick={() => setOpen((value) => !value)} className="ml-2 h-9 w-9 shrink-0 rounded-lg text-lg font-black text-slate-600 hover:bg-slate-100" aria-label={open ? "Recolher nível" : "Expandir nível"}>{open ? "−" : "+"}</button> : null}<Link href={href} className="flex min-w-0 flex-1 items-start sm:items-center"><FreeStudyLabel name={node.nome} count={node.count} framed={false} /></Link></div></div>{open ? node.children.map((child) => <StructureTreeNode key={child.id} node={child} slug={slug} recorteId={recorteId} depth={depth + 1} />) : null}</div>; }
function RootDeck({ study, slug, count, recorteId }: { study: LawStudyData; slug: string; count: number; recorteId: string | null }) { const href = `/questoes/${encodeURIComponent(slug)}/estudar?livre=1${recorteId ? `&recorte_id=${encodeURIComponent(recorteId)}` : ""}`; return <div className="flex min-h-16 min-w-0 items-start gap-2 px-3 py-3 sm:items-center sm:gap-3 sm:px-4"><Link href={href} className="group flex min-w-0 flex-1 items-start gap-2 rounded-lg py-1 sm:items-center sm:gap-3"><FreeStudyLabel name={study.law.title} count={count} /></Link></div>; }

function Materials({ study, settings, publicMode, autoOpen = false }: { study: LawStudyData; settings: AnkiTutorialSettings | null; publicMode: boolean; autoOpen?: boolean }) {
  const [open, setOpen] = useState(autoOpen); const [downloading, setDownloading] = useState<number | null>(null);
  ...
  return <section className="min-w-0 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:p-7"><h2 className="text-2xl font-black text-[#062a5f]">Materiais</h2><p className="mt-2 text-slate-600">{flashcard ? "Materiais complementares desta lei. Prefere estudar no Anki?" : "O deck do Anki ainda não está disponível para esta lei. Você pode consultar o tutorial do Anki enquanto isso."}</p><div className="mt-5 grid gap-3 sm:flex sm:flex-wrap">{flashcard ? <button type="button" onClick={() => setOpen(true)} className="min-h-12 w-full rounded-xl border border-blue-700 px-5 py-3 font-black text-blue-700 hover:bg-blue-50 sm:w-auto">Estudar no Anki</button> : <Link href="/estudar/anki" className="inline-flex min-h-12 w-full items-center justify-center rounded-xl border border-slate-300 px-5 py-3 font-black text-slate-800 hover:bg-slate-50 sm:w-auto">Ver tutorial do Anki</Link>}{pdf ? <button type="button" disabled={downloading === pdf.id} onClick={() => void download(pdf)} className="min-h-12 w-full rounded-xl border border-slate-300 px-5 py-3 font-black text-slate-800 hover:bg-slate-50 disabled:opacity-50 sm:w-auto">{downloading === pdf.id ? "Baixando…" : "Baixar PDF"}</button> : null}</div>{open && flashcard ? <AnkiModal settings={settings} onClose={() => setOpen(false)} onDownload={() => void download(flashcard)} downloading={downloading === flashcard.id} /> : null}</section>;
}
