...
export function RecordsPage({ contests }: { contests: RecordsContest[] }) {
  const groups = [
    { key: "edital", eyebrow: "CONCURSOS", title: "Concursos", items: contests.filter((contest) => contest.productType === "edital") },
  ...
  return <main className="min-h-screen overflow-hidden bg-[#020817] px-4 py-6 text-slate-100 sm:px-6 sm:py-10 lg:py-16">
    <div className="mx-auto max-w-6xl">
      ...
      {contests.length ? <div className="mt-9 space-y-10">{groups.filter((group) => group.items.length).map((group) => <section key={group.key} aria-labelledby={`records-${group.key}`}>
        <div><p className="font-mono text-[11px] font-black tracking-[.3em] text-cyan-300">{group.eyebrow}</p><h2 id={`records-${group.key}`} className="mt-1 text-2xl font-black text-white sm:text-3xl">{group.title}</h2></div>
