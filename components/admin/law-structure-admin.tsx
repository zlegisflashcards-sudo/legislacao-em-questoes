...
export function LawStructureAdmin({ lawSlug, nodes, onReload, onSelectNode, onDeleteComplete, selectNodeLabel = "Cadastrar questão aqui" }: { lawSlug: string; nodes: AdminLawStructureNode[]; onReload: () => Promise<void>; onSelectNode?: (node: AdminLawStructureNode) => void; onDeleteComplete?: (ids: number[]) => void; selectNodeLabel?: string }) {
  ...
  async function confirmDelete() {
    ...
    visit(deleting.id);
    try { await api({ action: "excluir_estrutura", law_slug: lawSlug, id: deleting.id, confirmation: deleteConfirmation }); onDeleteComplete?.([...removedIds]); setDeleting(null); setDeleteConfirmation(""); await onReload(); setMessage("Estrutura, questões e campanhas afetadas excluídas definitivamente."); }
