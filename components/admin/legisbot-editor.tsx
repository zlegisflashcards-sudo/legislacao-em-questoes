...
export default function LegisBotEditor({ record }: { record: LegisBotComentario }) {
  ...
  return <>
    <form action={action} className="admin-editor-form">
      ...
      <header className="admin-detail-header"><div><Link href="/admin/legisbot" className="admin-back">← Voltar aos comentários</Link><h1>{record.titulo}</h1><p>{record.slug} · ordem {record.ordem} · ID {record.id}</p></div><div className="admin-save-actions"><Link className="admin-button secondary" href={publicUrl}>← Voltar ao comentário</Link><Link className="admin-button secondary" href={publicUrl} target="_blank">Abrir página pública</Link><button type="submit" className="admin-button secondary" disabled={pending || !dirty} onClick={() => { openAfterSave.current = true; }}>{pending ? "Salvando…" : "Salvar e visualizar"}</button><button type="submit" className="admin-button primary" disabled={pending || !dirty}>{pending ? "Salvando…" : "Salvar alterações"}</button></div></header>
