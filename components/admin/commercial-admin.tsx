...
function StudentsPanelCore({ laws, products, rows, filter, setFilter }: { laws: Row[]; products: Row[]; rows: Row[]; filter: string; setFilter: (value: string) => void }) {
  ...
  const [emailChange, setEmailChange] = useState<{ email: string; confirmation: string; preview: Row | null; removeEmptyAccount: boolean } | null>(null);
  ...
  async function saveStudent(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (nextEmail !== currentEmail) throw new Error("Use “Editar e-mail” para alterar o acesso com confirmação segura.");
  ...
  function beginStudentAccessEmailChange() {
    if (!student) return;
    setEmailChange({ email: text(student.email), confirmation: "", preview: null, removeEmptyAccount: false });
    setEmailChangeError("");
  }
  async function previewStudentAccessEmailChange() {
    if (!student || !emailChange) return;
    setBusy(true); setEmailChangeError("");
    try {
      const preview = await requestJson("/api/admin/comercial/alunos", { method: "POST", body: JSON.stringify({ action: "prever_troca_email_acesso", id: student.id, data: { email: emailChange.email } }) });
      setEmailChange({ ...emailChange, preview });
    } catch (caught) { setEmailChangeError(caught instanceof Error ? caught.message : "Não foi possível validar o novo e-mail."); }
    finally { setBusy(false); }
  }
  async function changeStudentAccessEmail() {
    ...
    setBusy(true); setError(""); setMessage("");
    try {
      const payload = createStudentEmailChangePayload(text(student.id), emailChange.email, emailChange.confirmation, emailChange.removeEmptyAccount);
  ...
  return <><section className="commercial-card">
    ...
    {student && !busy && !error ? <>
      ...
      {editing ? <form className="commercial-card commercial-form-grid" onSubmit={saveStudent}><h3>Editar dados cadastrais</h3><input name="nome" defaultValue={text(student.nome)} placeholder="Nome" /><input name="email" type="email" value={editForm.email} readOnly aria-readonly="true" placeholder="E-mail" required /><small>Para alterar o e-mail, use “Editar e-mail”.</small><input name="telefone" defaultValue={text(student.telefone)} placeholder="Telefone opcional" /><p><small>UUID e user_id são somente leitura e não são alterados.</small></p><button className="admin-button primary" disabled={busy}>Salvar dados</button></form> : null}
      ...
      {emailChange && student ? <div className="student-delete-modal" role="presentation"><section className="commercial-card commercial-form-grid" role="dialog" aria-modal="true" aria-label="Alterar e-mail do aluno"><h3>Alterar e-mail do aluno</h3>{!emailChange.preview ? <><p>O campo permanece protegido até a validação server-side de duplicidade.</p><label>Novo e-mail<input type="email" value={emailChange.email} onChange={(event) => setEmailChange({ ...emailChange, email: event.target.value, preview: null })} autoComplete="email" required /></label></> : <><p><strong>De:</strong><br />{text(student.email)}</p><p><strong>Para:</strong><br />{text(emailChange.preview.email)}</p><p>O ID do usuário será mantido e os vínculos do aluno serão preservados.</p>{text(emailChange.preview.status) === "empty_auth" ? <label><input type="checkbox" checked={emailChange.removeEmptyAccount} onChange={(event) => setEmailChange({ ...emailChange, removeEmptyAccount: event.target.checked })} /> Remover a conta sem dados que já utiliza este e-mail.</label> : null}{text(emailChange.preview.status) === "public_conflict" ? <p className="admin-alert error">Este e-mail pertence a outro aluno: {text(object(emailChange.preview.aluno).nome) || "Sem nome"} · {text(object(emailChange.preview.aluno).email)} · UID {text(object(emailChange.preview.aluno).user_id) || "sem Auth"}. Compras: {text(emailChange.preview.compras)} · Progresso: {text(emailChange.preview.progresso)} · Campanhas: {text(emailChange.preview.campanhas)}.</p> : null}{text(emailChange.preview.status) === "auth_with_data" ? <p className="admin-alert error">Este e-mail pertence a outra conta que possui dados vinculados (UID {text(emailChange.preview.user_id)}). A alteração não foi realizada.</p> : null}{["public_conflict", "auth_with_data"].includes(text(emailChange.preview.status)) ? null : <label>Digite <strong>ALTERAR</strong> para confirmar<input value={emailChange.confirmation} onChange={(event) => setEmailChange({ ...emailChange, confirmation: event.target.value })} autoComplete="off" required /></label>}</>}{emailChangeError ? <p className="admin-alert error" role="alert">{emailChangeError}</p> : null}<div className="commercial-form-actions"><button type="button" className="admin-button secondary" disabled={busy} onClick={() => { setEmailChange(null); setEmailChangeError(""); }}>Cancelar</button>{!emailChange.preview ? <button type="button" className="admin-button primary" disabled={busy || emailChange.email.trim().toLowerCase() === text(student.email).trim().toLowerCase()} onClick={() => void previewStudentAccessEmailChange()}>{busy ? "Validando…" : "Continuar"}</button> : <button type="button" className="admin-button primary" disabled={busy || !["available", "empty_auth"].includes(text(emailChange.preview.status)) || emailChange.confirmation !== "ALTERAR" || (text(emailChange.preview.status) === "empty_auth" && !emailChange.removeEmptyAccount)} onClick={() => void changeStudentAccessEmail()}>{busy ? "Atualizando…" : "Salvar alteração"}</button>}</div></section></div> : null}
      <h3>Aquisições</h3>
...
function ProductPanel({ rows, laws, editing, setEditing, busy, mutate }: PanelProps & { laws: Row[] }) {
  ...
  const [recordsEnabled, setRecordsEnabled] = useState(Boolean(editing?.records_enabled));
  const [imageUrl, setImageUrl] = useState(text(editing?.imagem_url));
  const [uploadingImage, setUploadingImage] = useState(false);
  const [imageError, setImageError] = useState("");
  useEffect(() => { setProductType(text(editing?.tipo_produto) || "lei_avulsa"); setRecordsEnabled(Boolean(editing?.records_enabled)); setImageUrl(text(editing?.imagem_url)); setImageError(""); }, [editing]);
