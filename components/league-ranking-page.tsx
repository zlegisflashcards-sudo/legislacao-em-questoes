...
export function LeagueRankingPage({ initial, records = false }: { initial: LeagueRankingData | RecordsRankingData; records?: boolean }) {
  ...
  const recordsData = data as RecordsRankingData;
  ...
  return <main className="min-h-screen bg-[#020817] px-4 py-10 text-slate-100 sm:px-6 lg:py-16">
    <div className="mx-auto max-w-5xl">
      {records ? <section className="relative isolate overflow-hidden rounded-[2rem] border border-cyan-300/35 bg-[#031126] px-6 py-8 shadow-[0_0_70px_rgba(14,165,233,.16)] sm:px-10 sm:py-10"><div aria-hidden="true" className="absolute inset-0 -z-10 opacity-60 [background-image:linear-gradient(rgba(56,189,248,.06)_1px,transparent_1px),linear-gradient(90deg,rgba(56,189,248,.06)_1px,transparent_1px)] [background-size:26px_26px]" /><Link href="/recordes" className="inline-flex text-sm font-bold text-cyan-300 underline underline-offset-4">← Voltar para Records</Link><div className="mt-6 flex items-center gap-4 sm:gap-5"><RecordsContestImage src={recordsData.contest.contestImageUrl} alt={`Imagem ${recordsData.contest.shortName}`} className="h-16 w-16 shrink-0 rounded-2xl border border-cyan-200/25 object-cover object-right sm:h-20 sm:w-20" /><div><p className="font-mono text-[10px] font-black tracking-[.26em] text-cyan-300">UNIDADE DE ELITE</p><h1 className="mt-1 text-4xl font-black tracking-tight text-white sm:text-5xl">{recordsData.contest.shortName}</h1><p className="mt-1 text-base text-slate-300">Ranking Legis Questões</p></div></div></section> : <section className="relative isolate min-h-[23rem] overflow-hidden rounded-[2rem] border border-cyan-300/35 bg-[#031126] px-6 py-10 shadow-[0_0_70px_rgba(14,165,233,.16)] sm:min-h-[26rem] sm:px-10 sm:py-14">
      ...
      <section className="mt-6 rounded-[2rem] border border-cyan-300/30 bg-[#071329] p-4 shadow-[0_0_42px_rgba(6,182,212,.1)] sm:mt-8 sm:p-7">
        <div className="flex flex-wrap items-end justify-between gap-3 border-b border-cyan-200/20 pb-5">
          <div><p className="font-mono text-xs font-black tracking-[.3em] text-cyan-300">HIGH SCORES</p><h2 className="mt-1 text-xl font-black text-white sm:text-2xl">{records ? recordsData.contest.productName : data.league.name}</h2></div>
        </div>
