...
export default async function AdminLawsPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  return <main className="admin-shell commercial-admin-shell">
    ...
    <form className="commercial-card flex flex-col gap-3 sm:flex-row sm:items-end" action="/admin/leis">
      <label className="min-w-0 flex-1">Pesquisar lei<input name="q" defaultValue={query} placeholder="Título, slug, nome curto ou código" /></label>
    ...
    <section className="mt-5 grid gap-3" aria-label="Leis cadastradas">
      {laws.map((law) => <article key={law.id} className="commercial-card flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><h2 className="break-words">{law.titulo}</h2><p className="break-all text-sm text-slate-600">{law.codigo ? `${String(law.codigo)} · ` : ""}{law.slug} · {law.ativo ? "Ativa" : "Inativa"}</p></div><Link className="admin-button primary shrink-0" href={`/admin/leis/${encodeURIComponent(law.slug)}`}>Abrir central</Link></article>)}
