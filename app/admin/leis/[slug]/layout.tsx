...
export default async function AdminLawLayout({ children, params }: { children: React.ReactNode; params: Promise<{ slug: string }> }) {
  ...
  const navigation = () => <><Link data-law-nav="overview" href={base}>Visão geral</Link><Link data-law-nav="dados" href={`${base}/dados`}>Dados da lei</Link><Link data-law-nav="estrutura" href={`${base}/estrutura`}>Estrutura</Link><Link data-law-nav="recortes" href={`${base}/recortes`}>Recortes</Link><Link data-law-nav="legiscast" href={`${base}/legiscast`}>LegisCast</Link><Link data-law-nav="questoes" href={`${base}/questoes`}>Questões</Link><Link data-law-nav="anki" href={`${base}/anki`}>Anki</Link><Link data-law-nav="materiais" href={`${base}/materiais`}>Materiais</Link></>;
