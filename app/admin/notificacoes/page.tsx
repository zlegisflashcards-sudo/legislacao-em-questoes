...
const typeLabel = (type: string) => ({ novo_comentario: "Comentário", resposta_comentario: "Resposta", denuncia_comentario: "⚠ Denúncia", erro_resend: "E-mail", erro_hotmart: "Hotmart", aluno_duplicado: "Aluno duplicado", nova_aquisicao: "Aquisição", nova_liberacao: "Liberação", conta_ativada: "Conta ativada", reembolso_solicitado: "Reembolso", reembolso_concluido: "Reembolso", acesso_restaurado: "Acesso restaurado" }[type] ?? type);
...
export default async function AdminNotificationsPage({ searchParams }: { searchParams: Promise<Query> }) {
  ...
  if (result.error) {
    console.error("[admin-notifications] load_failed", { code: result.error.code, message: result.error.message, details: result.error.details, hint: result.error.hint });
