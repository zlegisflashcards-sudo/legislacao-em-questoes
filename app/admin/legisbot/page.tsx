...
export default async function AdminLegisBotPage({ searchParams }: { searchParams: Promise<Query> }) {
  ...
  return <main className="admin-shell">
    <header className="admin-header"><div><div className="admin-eyebrow">Administração</div><h1>Comentários do LegisBot</h1><p>{count ?? 0} registro(s) encontrado(s)</p></div><div className="admin-header-actions"><span>{user.email}</span><form action={sairAdministrador}><button className="admin-button secondary">Sair</button></form></div></header>
