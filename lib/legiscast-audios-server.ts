...
import { sortLegiscastAudios } from "@/lib/legiscast-audio-order";
...
export async function listAuthorizedLegiscastAudios(request: Request, slug: string) {
  ...
  const { data, error } = await db.from("legiscast_audios").select("id,titulo,descricao,duracao_segundos,ordem,storage_path,created_at").eq("lei_id", context.lawId).eq("ativo", true);
  if (error) throw new LawStudyApiError(503, "Não foi possível carregar os áudios desta lei.");
  const audios = await Promise.all(sortLegiscastAudios(data ?? []).map(async (audio) => {
