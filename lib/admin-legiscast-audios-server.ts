...
function fields(input: AuthorizeInput) {
  ...
  if (!fileName || !isAcceptedLegiscastOriginal(fileName, mime)) throw new AdminLegiscastAudioError(400, "Aceitamos somente arquivos MP3, M4A, WAV ou MP4.");
  if (!sizeBytes) throw new AdminLegiscastAudioError(400, "Selecione um arquivo de áudio ou vídeo com faixa de áudio."); if (sizeBytes > LEGISCAST_ORIGINAL_MAX_BYTES) throw new AdminLegiscastAudioError(400, "O arquivo original deve ter no máximo 500 MB.");
...
function statusLabel(status: string) { return ({ pendente: "Na fila", processando: "Processando", concluido: "Concluído", erro: "Erro", cancelado: "Cancelado" } as Record<string, string>)[status] ?? status; }
...
export async function listAdminLegiscastAudios(lawSlug?: string | null) {
  ...
  let jobsRequest = db.from("legiscast_audio_jobs").select("id,lei_id,structure_id,titulo,status,original_size_bytes,final_path,final_size_bytes,mp3_path,mp3_size_bytes,duracao_segundos,erro_codigo,erro_mensagem,tentativas,created_at,leis(titulo,slug)").order("created_at", { ascending: false }).limit(30);
  ...
  const audioPaths = (audios.data ?? []).map((audio) => audio.storage_path);
  const mp3Jobs = audioPaths.length
    ? await db.from("legiscast_audio_jobs").select("final_path").eq("status", "concluido").not("mp3_path", "is", null).in("final_path", audioPaths)
    : { data: [], error: null };
  if (mp3Jobs.error) throw new AdminLegiscastAudioError(503, "Não foi possível carregar as versões MP3 do LegisCast.");
  const mp3ByFinalPath = new Set((mp3Jobs.data ?? []).map((job) => job.final_path));
  return { laws: laws.data ?? [], audios: (audios.data ?? []).map((audio) => ({ ...audio, mp3_available: mp3ByFinalPath.has(audio.storage_path) })), structures: structures.data ?? [], jobs: (jobs.data ?? []).map((job) => ({ ...job, statusLabel: statusLabel(job.status) })), originalMaxBytes: LEGISCAST_ORIGINAL_MAX_BYTES, finalMaxBytes: 50 * 1024 * 1024 };
}
...
export async function confirmAdminLegiscastOriginal(input: ConfirmInput) {
  ...
  if (error || !job || job.original_path !== operation.originalPath || job.original_bucket !== getLegiscastOriginalBucketName()) throw new AdminLegiscastAudioError(404, "Processamento não encontrado."); if (job.status === "cancelado") throw new AdminLegiscastAudioError(409, "Este processamento foi cancelado e não pode ser retomado."); if (job.status === "concluido" || job.status === "processando") return { jobId, status: job.status };
  ...
  const queued = await db.from("legiscast_audio_jobs").update({ status: "pendente", updated_at: new Date().toISOString() }).eq("id", jobId).in("status", ["pendente", "erro"]).select("id").maybeSingle(); if (queued.error || !queued.data) throw new AdminLegiscastAudioError(409, "O processamento não está disponível para iniciar.");
...
function downloadFileName(lawTitle: string, title: string | null, path: string) { const extension = path.split(".").pop()?.toLowerCase().replace(/[^a-z0-9]/g, "") || "m4a"; const clean = (value: string) => value.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-zA-Z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 100) || "LegisCast"; return `${clean(lawTitle)}-${clean(title || "LegisCast")}.${extension}`; }
async function audioForAdmin(audioId: unknown) { const id = String(audioId ?? ""); if (!/^[0-9a-f-]{36}$/i.test(id)) throw new AdminLegiscastAudioError(400, "Áudio inválido."); const db = getSupabaseServerClient(); const result = await db.from("legiscast_audios").select("id,lei_id,titulo,storage_path").eq("id", id).maybeSingle(); if (result.error || !result.data) throw new AdminLegiscastAudioError(404, "Áudio não encontrado."); const law = await db.from("leis").select("titulo").eq("id", result.data.lei_id).maybeSingle(); if (law.error || !law.data) throw new AdminLegiscastAudioError(404, "Lei do áudio não encontrada."); return { db, audio: result.data, law: law.data }; }
...
export async function downloadAdminLegiscastAudio(input: AudioInput) { await requireAdmin(); const { db, audio, law } = await audioForAdmin(input.audioId); const signed = await db.storage.from("legiscast-audio").createSignedUrl(audio.storage_path, 120, { download: downloadFileName(law.titulo, audio.titulo, audio.storage_path) }); if (signed.error || !signed.data?.signedUrl) throw new AdminLegiscastAudioError(503, "Não foi possível preparar o download."); return { url: signed.data.signedUrl }; }
export async function downloadAdminLegiscastMp3(input: AudioInput) { await requireAdmin(); const { db, audio, law } = await audioForAdmin(input.audioId); const job = await db.from("legiscast_audio_jobs").select("mp3_path").eq("final_path", audio.storage_path).eq("status", "concluido").not("mp3_path", "is", null).maybeSingle(); if (job.error || !job.data?.mp3_path) throw new AdminLegiscastAudioError(404, "A versão MP3 ainda não está disponível para este áudio."); const signed = await db.storage.from("legiscast-audio").createSignedUrl(job.data.mp3_path, 120, { download: downloadFileName(law.titulo, audio.titulo, job.data.mp3_path) }); if (signed.error || !signed.data?.signedUrl) throw new AdminLegiscastAudioError(503, "Não foi possível preparar o download MP3."); return { url: signed.data.signedUrl }; }
