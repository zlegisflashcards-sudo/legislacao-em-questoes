...
describe("gestão definitiva de questões e estruturas", () => {
  ...
  it("exige confirmação para campanhas de outro aluno", () => { expect(migration).toContain("v_has_other_student"); expect(migration).toContain("p_confirmation is distinct from 'EXCLUIR'"); expect(central).toContain("digite <strong>EXCLUIR</strong>"); expect(structureAdmin).toContain("Digite <strong>EXCLUIR</strong>"); });
