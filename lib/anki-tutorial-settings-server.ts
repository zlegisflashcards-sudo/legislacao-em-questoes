...
export async function getAnkiTutorialSettings(): Promise<AnkiTutorialSettings | null> {
  ...
  try {
    const { data, error } = await getSupabaseServerClient()
      ...
      .select("computador_app_url,computador_tutorial_url,android_app_url,android_tutorial_url,ios_app_url,ios_tutorial_url,navegador_app_url,navegador_tutorial_url,tutorial_questoes_url,computador_estudo_url,android_estudo_url,ios_estudo_url,navegador_estudo_url")
      .eq("id", 1)
      .maybeSingle();
